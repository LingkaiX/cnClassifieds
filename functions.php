...
function add_styles_and_scripts() {
	...
	wp_enqueue_script( 'my-js', get_template_directory_uri() . '/js/myjs.js', array('jquery'), '1.0.2', true );
...
function putAbnSignal($hasAbn){
	if($hasAbn){
		...
		.get_template_directory_uri().'/img/ABN-CHECKED.svg"></img></abbr>';
	}else{
		return null;
	}
}
...
function getBaseUrl(){
	$url=get_site_url();
	if (parsePath($_SERVER['REQUEST_URI'],'',-1)=='zh-tw'||$_GET["variant"]=='zh-tw'){
		return $url.'/zh-tw';
	}else{
		return $url;
	}
